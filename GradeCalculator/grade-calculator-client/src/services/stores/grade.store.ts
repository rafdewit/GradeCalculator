...
export class GradeStore {
  ...
  private get(): Observable<StudentCollection[]> {
    ...
    const attachCreate$ = initialClasses$.pipe(
      switchMap(r => {
        return this.gradeEventClient.studentCollectionUpdateEvent$.pipe(
          startWith(null),
          scan((acc, value) => {
            if (value) {
    ...
    const attachDelete$ = attachCreate$.pipe(
      switchMap(r => {
        return this.gradeEventClient.studentCollectionDeletedEvent$.pipe(
          startWith(null),
          scan((acc, id) => {
            if (id) {
