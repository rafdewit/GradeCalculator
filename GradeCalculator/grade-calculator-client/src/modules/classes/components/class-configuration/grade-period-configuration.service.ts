...
export class GradePeriodConfigurationService {
  constructor(
    private gradePeriodClient: IGradePeriodClient,
    private dialogService: DialogService,
    private matDialog: MatDialog
  ) {}

  public async openCreatePeriodDialog(
    studentCollection: StudentCollection
  ): Promise<void> {
    const result = await this.openPeriodDialog('Create Grade Period');
    if (result) {
      await firstValueFrom(
        this.gradePeriodClient.createGradePeriod({
  ...
  public async copyPeriod(
    gradePeriod: GradePeriod,
    studentCollection: StudentCollection
  ): Promise<void> {
    const result = await this.openPeriodDialog(
      `Copy Grade Period: ${gradePeriod.name}`,
      gradePeriod
    );
    if (result) {
      await firstValueFrom(
        this.gradePeriodClient.copyGradePeriod({
          studentCollectionId: studentCollection.id,
          name: result.name,
          weight: result.weight,
  ...
  public async updatePeriod(
    ...
    const result = await this.openPeriodDialog(
      `Update Grade Period: ${gradePeriod.name}`,
      gradePeriod
    );
    if (result) {
      await firstValueFrom(
        this.gradePeriodClient.updateGradePeriod({
          studentCollectionId: studentCollection.id,
          name: result.name,
          weight: result.weight,
          gradePeriodId: gradePeriod.id,
  ...
  public async deletePeriod(
    ...
    const dialogResult = await this.dialogService.openConfirmationDialogDialog(
      `Delete period: ${gradePeriod.name}?`,
      `Are you sure you want to delete period: ${gradePeriod.name}`
    );
    if (dialogResult) {
      await firstValueFrom(
        this.gradePeriodClient.deleteGradePeriod({
          studentCollectionId: studentCollection.id,
          gradePeriodId: gradePeriod.id,
        })
      );
  ...
  public async openPeriodDialog(
    title: string,
    period: GradePeriod | null = null
  ): Promise<CreatePeriodDialogResultData | null> {
    ...
    const input = new MatDialogConfig<
      DefaultCrudDialogData<CreatePeriodDialogData>
    >();
    input.data = data;
    ...
    const dialogRef = this.matDialog.open<
      ...
      DefaultCrudDialogData<CreatePeriodDialogData>,
      CreatePeriodDialogResultData
    >(CreatePeriodDialogComponent, input);
